...
const PracticePage = () => {
  ...
  const timeLimit = Number.parseInt(searchParams.get("timeLimit") || "5")
  ...
  const initialMode = (searchParams.get("mode") || "casual-chat") as
    | "casual-chat"
    | "speaking-practice"
    | "interview"
  ...
  // Convert realtime messages to ConversationMessage format for display
  const conversation: ConversationMessage[] = realtimeMessages.map((msg) => ({
    id: msg.id,
    role: msg.role,
    content: msg.content,
  ...
  const endCall = async (autoEnded = false) => {
    ...
    if (conversation.length > 0 && conversationId) {
      const actualTimeLimitMinutes = timeLimit
      const now = Date.now()
      const sessionDuration = (actualTimeLimitMinutes * 60 - timeRemaining) / 60
  ...
  }, [conversation, currentAITranscript, currentUserTranscript])
