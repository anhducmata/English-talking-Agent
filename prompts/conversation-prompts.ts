import { wrapWithChildSafeInstructions } from '@/lib/guardrails'
...
export interface ConversationConfig {
  ...
  enableChildSafety?: boolean // Default: true
}
...
export function generateConversationSystemPrompt(config: ConversationConfig): string {
  const { topic, difficulty, language, mode, interviewContext, enableChildSafety = true } = config
...
- Giữ phản hồi ngắn gọn và tập trung vào quá trình phỏng vấn`,
  ...
  const basePrompt = `${baseInstructions[language][mode]}
...
IMPORTANT: Respond directly without any role labels or prefixes. Just provide your natural response as if you're speaking directly to the person.`
  ...
  // Wrap with child safety instructions if enabled
  if (enableChildSafety) {
    return wrapWithChildSafeInstructions(basePrompt, language)
  }
